...
// Endpoints de PDF (antes do middleware global)
// Servir PDF da matrícula
router.get('/:id/matricula', authMiddleware, async (req, res) => {
  try {
...
// Servir PDF do CAR
router.get('/:id/car', authMiddleware, async (req, res) => {
  try {
    const property = await prisma.property.findUnique({
      where: { id: req.params.id },
      select: { car: true }
...
router.get('/:id/georreferenciamento', authMiddleware, async (req, res) => {
  try {
    const property = await prisma.property.findUnique({
      where: { id: req.params.id },
...
// Rota para atualizar status da propriedade (apenas ADMIN)
router.patch('/:id/status', [
  body('status').isIn(['PENDING', 'APPROVED', 'REJECTED', 'ACTIVE', 'INACTIVE']).withMessage('Status inválido')
], async (req, res) => {
  try {
    // Validar dados de entrada
    const errors = validationResult(req)
    ...
    res.json({
      success: true,
      message: `Propriedade ${status === 'APPROVED' ? 'aprovada' : status === 'REJECTED' ? 'rejeitada' : 'atualizada'} com sucesso`,
      data: updatedProperty
    })
  } catch (error) {
    console.error('Erro ao atualizar status da propriedade:', error)
