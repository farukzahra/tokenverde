    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
        <div class="card">
          <div class="flex items-center">
            ...
            <div class="ml-3 sm:ml-4">
              <h3 class="text-base sm:text-lg font-semibold text-gray-900">Meus Tokens</h3>
              <p class="text-xl sm:text-2xl font-bold text-green-600">{{ totalTokens }}</p>
              <p class="text-xs sm:text-sm text-gray-600">Valor: R$ {{ totalValue.toFixed(2) }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="card">
          <div class="flex items-center">
            ...
            <div class="ml-3 sm:ml-4">
              <h3 class="text-base sm:text-lg font-semibold text-gray-900">Áreas Investidas</h3>
              <p class="text-xl sm:text-2xl font-bold text-blue-600">{{ investedAreas }}</p>
              <p class="text-xs sm:text-sm text-gray-600">{{ totalArea.toFixed(1) }} hectares</p>
            </div>
          </div>
        </div>
        ...
        <div class="card sm:col-span-2 lg:col-span-1">
          <div class="flex items-center">
            ...
            <div class="ml-3 sm:ml-4">
              <h3 class="text-base sm:text-lg font-semibold text-gray-900">Transações</h3>
              <p class="text-xl sm:text-2xl font-bold text-purple-600">{{ totalTransactions }}</p>
              <p class="text-xs sm:text-sm text-gray-600">Último mês</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
        ...
        <div v-if="user.role === 'ADMIN'" class="card">
          <h3 class="text-base sm:text-lg font-semibold mb-4">Painel Administrativo</h3>
          <div class="space-y-4">
            <div class="grid grid-cols-2 gap-3 sm:gap-4">
              <div class="text-center p-3 sm:p-4 bg-yellow-50 rounded-lg">
                <div class="text-xl sm:text-2xl font-bold text-yellow-600">{{ adminStats.pending }}</div>
                <div class="text-xs sm:text-sm text-gray-600">Pendentes</div>
              </div>
              <div class="text-center p-3 sm:p-4 bg-green-50 rounded-lg">
                <div class="text-xl sm:text-2xl font-bold text-green-600">{{ adminStats.approved }}</div>
                <div class="text-xs sm:text-sm text-gray-600">Aprovadas</div>
              </div>
            </div>
            <div class="space-y-2">
              <button 
                @click="$router.push('/admin')"
                class="w-full btn-primary text-sm sm:text-base"
              >
                Gerenciar Propriedades
              </button>
              <button 
                @click="$router.push('/users')"
                class="w-full btn-secondary text-sm sm:text-base"
              >
                Gerenciar Usuários
              </button>
              <button 
        ...
        <div v-if="user.role === 'INVESTIDOR'" class="card">
          <h3 class="text-base sm:text-lg font-semibold mb-4">Meus Tokens Verdes</h3>
          <div v-if="loading" class="text-center py-4">
            <p class="text-gray-600 text-sm sm:text-base">Carregando...</p>
          </div>
          <div v-else-if="tokenHolders.length === 0" class="text-center py-6 sm:py-8">
            <p class="text-gray-600 mb-4 text-sm sm:text-base">Você ainda não possui tokens</p>
            <button @click="$router.push('/map')" class="btn-primary text-sm sm:text-base">
          ...
          <div v-else class="space-y-3">
            <div v-for="holder in tokenHolders" :key="holder.id" class="border rounded-lg p-3">
              <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-2 sm:space-y-0">
                <div>
                  <h4 class="font-medium text-sm sm:text-base">{{ holder.token.name }}</h4>
                  <p class="text-xs sm:text-sm text-gray-600">{{ holder.token.symbol }}</p>
                </div>
                <div class="text-left sm:text-right">
                  <p class="font-semibold text-sm sm:text-base">{{ holder.quantity }} tokens</p>
                  <p class="text-xs sm:text-sm text-gray-600">R$ {{ (holder.quantity * holder.token.price).toFixed(2) }}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div v-if="user.role === 'PROPERTY_OWNER'" class="card">
          <h3 class="text-base sm:text-lg font-semibold mb-4">Minhas Propriedades</h3>
          <div v-if="loading" class="text-center py-4">
            <p class="text-gray-600 text-sm sm:text-base">Carregando...</p>
          </div>
          <div v-else-if="properties.length === 0" class="text-center py-6 sm:py-8">
            <p class="text-gray-600 mb-4 text-sm sm:text-base">Você ainda não cadastrou propriedades</p>
            <button @click="$router.push('/property-form')" class="btn-primary text-sm sm:text-base">
              Cadastrar Propriedade
            </button>
          </div>
          <div v-else class="space-y-3">
            <div v-for="property in properties" :key="property.id" class="border rounded-lg p-3">
              <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-2 sm:space-y-0">
                <div>
                  <h4 class="font-medium text-sm sm:text-base">{{ property.name }}</h4>
                  <p class="text-xs sm:text-sm text-gray-600">{{ property.area }} hectares</p>
                </div>
        ...
        <!-- Transações Recentes -->
        <div class="card">
          <h3 class="text-base sm:text-lg font-semibold mb-4">Transações Recentes</h3>
          <div v-if="loading" class="text-center py-4">
            <p class="text-gray-600 text-sm sm:text-base">Carregando...</p>
          </div>
          <div v-else-if="transactions.length === 0" class="text-center py-6 sm:py-8">
            <p class="text-gray-600 text-sm sm:text-base">Nenhuma transação encontrada</p>
          </div>
          <div v-else class="space-y-3">
            <div v-for="transaction in transactions.slice(0, 5)" :key="transaction.id" class="border rounded-lg p-3">
              <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-2 sm:space-y-0">
                <div>
                  <h4 class="font-medium text-sm sm:text-base">{{ transaction.token.symbol }}</h4>
                  <p class="text-xs sm:text-sm text-gray-600">{{ getTransactionTypeLabel(transaction.type) }}</p>
                </div>
                <div class="text-left sm:text-right">
                  <p class="font-semibold text-sm sm:text-base">{{ transaction.quantity }} tokens</p>
                  <p class="text-xs sm:text-sm text-gray-600">R$ {{ transaction.amount.toFixed(2) }}</p>
                </div>
              </div>
            </div>
          </div>
...
const adminStats = ref({ pending: 0, approved: 0, rejected: 0, total: 0 })
